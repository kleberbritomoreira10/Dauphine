...
void PStateIdle::enter(){
	...
	this->player->get_animation()->change_animation(0, 0, 26, false, 2.6);
...
void PStateIdle::handleInput(const std::array<bool, GameKeys::MAX> keyStates_){
	...
	if(keyStates_[GameKeys::LATTACK]){
		this->player->change_state(Player::player_states::ATTACK);
		return;
	}

    if(keyStates_[GameKeys::ACTION]){
    	// Game::instance().get_audio_handler().addSoundEffect("res/audio/FX_NADINE/FX_AAAAAHH.wav");
	...
	if(keyStates_[GameKeys::AIM]){
		this->player->change_state(Player::player_states::AIMING);
		return;
	}
